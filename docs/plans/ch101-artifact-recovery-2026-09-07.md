...
5. Only after a real source is preserved: `scripts/ai3d/review_recovered_spar3d_artifact.py` chains Blender refine → normalization → corrected texture mapping → new renders → score → strict visual QA and ranking. It requires source mesh and reference manifest SHA256, discovers Blender and the optional `xvfb-run` prefix without notebook globals, and never exports a Unity package. Semantic/visual inspection is still required even if numerical thresholds pass.
