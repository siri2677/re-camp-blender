...
6. `/content/re-camp-CH101-blockout-v006.zip`을 브라우저로 다운로드한다.

Colab 세션이 끝나면 `/content`의 파일은 삭제되므로 ZIP을 세션 종료 전에 다운로드해야 한다.

## 판정 경계

현재 자동화 결과는 v005 기술 준비 위에 humanoid-aligned armature 프로토타입, socket-to-bone 연결, idle/run/attack/A-pose 검토 클립을 추가한 Rig and Motion Preparation v006이다. 스킨 웨이트는 적용하지 않았으며, 최종 캐릭터 모델, Unity Import, Android 성능, Gate B 승인을 의미하지 않는다.
